...
class MBSource(MediaSource):
    ...
    async def _set_hubs(self):
        ...
        if entries[0].entry_id in self._hass.data[DOMAIN]:
            hubs = [
                self._hass.data[DOMAIN][entry.entry_id][DATA_HUB] for entry in entries
            ]
    ...
    async def async_browse_media(self, item: MediaSourceItem) -> BrowseMediaSource:
        ...
        if not self.hubs and not await self._set_hubs():
